...
namespace _PrimeiroWebService.Controllers
{
    public class CrudController
    {
        ...
        /// <param name="model">Objeto do tipo Model</param>
        /// <param name="tableName">Nome da tabela</param>
        public void UpdateModel(object whereModel)
        {
        ...
        {
            Recordset rs = (Recordset)ServerConnections.company.GetBusinessObject(BoObjectTypes.BoRecordset);

            string sql = @"SELECT DocEntry FROM [{0}] WHERE {1}";
            sql = ServerConnections.TranslateToHana(String.Format(sql, TableName, where));

            rs.DoQuery(String.Format(sql, TableName, where));
            if (rs.RecordCount > 0)
            {
                CompanyService scompany = ServerConnections.company.GetCompanyService();
        ...
        private void SaveModel(EnumCrudOperation enumCrudOperation)
        {
            ...
            try
            {
                scompany = ServerConnections.company.GetCompanyService();
                ...
                if (enumCrudOperation == EnumCrudOperation.Update)
                {
                    ...
                    {
                        Recordset rstDocEntry = (Recordset)ServerConnections.company.GetBusinessObject(BoObjectTypes.BoRecordset);
                        ...
                        sql = ServerConnections.TranslateToHana(sql);
        ...
        {
            UserTable utbUser = ServerConnections.company.UserTables.Item(TableName.Replace("@", ""));
        ...
        public string RetrieveModelSql(Type modelType, string where, string orderBy, bool getValidValues)
        {
            ...
            if (getValidValues)
            {
                ...
                Recordset rstValidValues = (Recordset)ServerConnections.company.GetBusinessObject(BoObjectTypes.BoRecordset);
                sqlValidValues = ServerConnections.TranslateToHana(sqlValidValues);
            ...
            foreach (PropertyInfo property in modelType.GetProperties())
            {
                foreach (Attribute attribute in property.GetCustomAttributes(true))
                {
                    ...
                    if (!getValidValues || !fieldValidValues.ContainsKey(modelController.ColumnName.Replace("U_", "")))
                    {
                        try
                        {
                            if (ServerConnections.company.DbServerType == (BoDataServerTypes)9)
                            {
        ...
        public T RetrieveModelByKey<T>(string key)
        {
            ...
            ModelControllerAttribute modelController;
            UserTable utbModel = ServerConnections.company.UserTables.Item(TableName.Replace("@", ""));
        ...
        public List<T> RetrieveModelList<T>(string joinTable, string joinCondition, string where, string orderBy)
        {
            ...
            foreach (PropertyInfo property in modelType.GetProperties())
            {
                ...
                foreach (Attribute attribute in property.GetCustomAttributes(true))
                {
                    ...
                    if (modelController != null)
                    {
                        ServerConnections.company = db.GetCompany();
        ...
        public static string GetNextCode(string tableName, string fieldName, string where)
        {
            ...
            Recordset rs = (Recordset)ServerConnections.company.GetBusinessObject(BoObjectTypes.BoRecordset);
            sSql = ServerConnections.TranslateToHana(sSql);
            rs.DoQuery(sSql);
        ...
        public List<T> FillModel<T>(string sql)
        {
            ...
            try
            {
                ...
                rs = (Recordset)ServerConnections.company.GetBusinessObject(BoObjectTypes.BoRecordset);
                sql = ServerConnections.TranslateToHana(sql);
                rs.DoQuery(sql);
        ...
        public string Exists(string returnColumn, string where)
        {
            ...
            // Lê os dados em um Recordset
            Recordset rs = (Recordset)ServerConnections.company.GetBusinessObject(SAPbobsCOM.BoObjectTypes.BoRecordset);
            sql = ServerConnections.TranslateToHana(sql);
            rs.DoQuery(sql);
            if (rs.RecordCount > 0)
            {
